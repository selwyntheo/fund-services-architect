      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
        <div className="p-6 border-b border-gray-100">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-gray-500 text-sm mt-1">Upcoming event milestones and target dates</p>
            </div>
        ...
        <div className="p-6 overflow-x-auto">
          <div className="relative min-w-max">
            ...
            <div className="absolute top-12 left-0 right-0 h-0.5" style={{ backgroundColor: '#e7500d' }}></div>
            ...
            <div className="flex gap-6 pb-4">
              {events
                ...
                .map((event, idx) => {
                  ...
                  return (
                    <div key={event.id} className="relative flex-shrink-0" style={{ width: '280px' }}>
                      {/* Timeline Dot */}
                      <div className="flex justify-center mb-3">
                        <div 
                          className="w-6 h-6 rounded-full border-4 border-white shadow-sm flex items-center justify-center"
                          style={{ 
                      ...
                      {/* Date Badge */}
                      <div className="flex justify-center mb-3">
                        <div className="px-3 py-1.5 rounded-lg text-xs font-semibold text-white shadow-sm" style={{ backgroundColor: '#e7500d' }}>
                          {new Date(event.targetDate).toLocaleDateString('en-US', { month: 'short', day: 'numeric' })}
                        </div>
                      </div>
                      ...
                      <div 
                        ...
                        style={{ borderColor: isCompleted ? '#10b981' : isUrgent ? '#e7500d' : '#3b82f6' }}
                        onClick={() => {
                      ...
                      >
                        <div className="mb-3">
                          <div className="flex items-start gap-2 mb-2">
                            <Package className="w-4 h-4 flex-shrink-0 mt-0.5" style={{ color: '#e7500d' }} />
                            <h3 className="font-semibold text-gray-900 text-sm leading-tight">{event.eventName}</h3>
                          </div>
                          {isUrgent && !isCompleted && (
                            <span className="inline-block px-2 py-0.5 rounded text-xs font-medium mb-2" style={{ backgroundColor: '#fef3f0', color: '#e7500d' }}>
                              {daysUntil} days left
                            </span>
                          )}
                          {isCompleted && (
                            <span className="inline-block px-2 py-0.5 rounded text-xs font-medium bg-emerald-50 text-emerald-700 mb-2">
                              Completed
                            </span>
                          )}
                        </div>
                        ...
                        <div className="space-y-2 mb-3">
                          <div className="flex items-center gap-1.5 text-xs text-gray-600">
                            ...
                            <span>{event.source} → Eagle</span>
                          </div>
                          <div className="flex items-center gap-1.5 text-xs text-gray-600">
                            <Users className="w-3 h-3" />
                            <span>{event.funds.length} funds</span>
                          </div>
                          <div className="flex items-center gap-1.5 text-xs text-gray-600">
                            <DollarSign className="w-3 h-3" />
                            <span>${(event.totalAUM / 1000000).toFixed(0)}M AUM</span>
                          </div>
                        </div>
                          ...
                          <div className="w-full bg-gray-100 rounded-full h-1.5 overflow-hidden">
                            <div 
                              ...
                              style={{ 
                                ...
                                backgroundColor: isCompleted ? '#10b981' : '#e7500d'
                              }}
                            ></div>
                          </div>
                        </div>
        ...
        <div className="border-t border-gray-100 p-6 bg-gray-50">
          <div className="flex items-center justify-between">
            <div>
              <h3 className="text-sm font-semibold text-gray-900 mb-1">Next Milestone</h3>
              {(() => {
                const nextEvent = events
                  .filter(e => e.progress < 100)
                  .sort((a, b) => new Date(a.targetDate) - new Date(b.targetDate))[0];
                
                if (!nextEvent) return <p className="text-sm text-gray-500">No upcoming milestones</p>;
                
                const daysUntil = Math.ceil((new Date(nextEvent.targetDate) - new Date()) / (1000 * 60 * 60 * 24));
                
                return (
                  <p className="text-sm text-gray-600">
                    <span className="font-semibold" style={{ color: '#e7500d' }}>{nextEvent.eventName}</span> in {daysUntil} days
                  </p>
                );
              })()}
            </div>
            <div className="flex items-center gap-3">
              <div className="text-right">
                <p className="text-xs text-gray-500">Timeline Summary</p>
                <div className="flex items-center gap-4 mt-1">
                  <span className="text-xs text-gray-600">
                    <strong style={{ color: '#e7500d' }}>
                      {events.filter(e => {
                        const days = Math.ceil((new Date(e.targetDate) - new Date()) / (1000 * 60 * 60 * 24));
                        return days <= 30 && e.progress < 100;
                      }).length}
                    </strong> Urgent
                  </span>
                  <span className="text-xs text-gray-600">
